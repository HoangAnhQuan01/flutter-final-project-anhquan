...
## Báo cáo kết quả
# Báo cáo Kết quả Bài tập lớn: Ứng dụng Quản lý Vé xem phim

## 1. Tổng quan Dự án
Ứng dụng **Movie Tickets** là một ứng dụng di động được phát triển bằng **Flutter**, phục vụ việc quản lý thông tin phim và vé xem phim. Ứng dụng tích hợp các chức năng quản lý dữ liệu (CRUD) và xác thực người dùng thông qua nền tảng **Firebase**.

## 2. Các Thư viện và Công nghệ sử dụng
Dự án sử dụng các gói thư viện chính sau:

* **Flutter SDK**: Framework chính để xây dựng giao diện người dùng đa nền tảng.
* **firebase_auth**: Quản lý xác thực người dùng (Đăng nhập, Đăng ký, Đăng xuất).
* **cloud_firestore**: Cơ sở dữ liệu NoSQL đám mây để lưu trữ thông tin phim.
* **flutter_test**: Thư viện hỗ trợ kiểm thử đơn vị (Unit Test) và kiểm thử Widget.

## 3. Mô tả Chức năng đã triển khai

### 3.1. Xác thực Người dùng (Authentication)
Hệ thống xác thực được xây dựng dựa trên `FirebaseAuth` với các tính năng:
* **Đăng ký (`RegisterScreen`):**
    * Cho phép người dùng tạo tài khoản mới.
    * Kiểm tra tính hợp lệ của mật khẩu xác nhận ngay trên giao diện.
    * Xử lý và thông báo lỗi từ phía server (ví dụ: email đã tồn tại).
* **Đăng nhập (`LoginScreen`):**
    * Xác thực người dùng bằng Email/Password.
    * Hiển thị thông báo lỗi chi tiết cho các trường hợp: `user-not-found` (Email không tồn tại), `wrong-password` (Sai mật khẩu), `invalid-email` (Email không hợp lệ).
* **Đăng xuất:** Tích hợp nút đăng xuất trong màn hình danh sách phim.

### 3.2. Quản lý Phim (Movie Management - CRUD)
Dữ liệu phim được quản lý thông qua lớp `MovieService` tương tác trực tiếp với Cloud Firestore.

* **Mô hình dữ liệu (`Movie`):**
    * Lưu trữ các thông tin: `id`, `title`, `duration`, `price`, `isShowing`, `ticketQuantity`, `coverImage`, `description`.
    * Sử dụng phương thức `fromMap` và `toMap` để chuyển đổi dữ liệu giữa Object và Firestore Document.

* **Các thao tác chính:**
    1.  **Xem danh sách (Read):** Sử dụng `StreamBuilder` tại `MovieListScreen` để hiển thị danh sách phim và cập nhật tự động khi dữ liệu thay đổi.
    2.  **Xem chi tiết (Read Detail):** Sử dụng `StreamBuilder` tại `MovieDetailScreen` lắng nghe luồng dữ liệu của một bộ phim cụ thể (`service.getMovie(id)`), giúp giao diện chi tiết tự động cập nhật ngay lập tức khi có chỉnh sửa.
    3.  **Thêm mới (Create):** Thực hiện tại `MovieFormScreen`.
    4.  **Cập nhật (Update):** Tái sử dụng `MovieFormScreen` với dữ liệu được điền sẵn. Logic xử lý phân biệt giữa thêm mới và cập nhật dựa trên việc đối tượng `movie` có tồn tại hay không.
    5.  **Xóa (Delete):** Cho phép xóa phim trực tiếp từ danh sách.

## 4. Các Kiểm thử đã thực hiện

### 4.1. Kiểm thử Đơn vị (Unit Test)
* **File:** `test/movie_test.dart`
* **Nội dung:** Kiểm tra phương thức `toMap()` của lớp `Movie`.
* **Mục đích:** Đảm bảo đối tượng `Movie` được chuyển đổi chính xác thành `Map<String, dynamic>` để lưu trữ lên Firestore, bao gồm các trường dữ liệu quan trọng như `title`, `duration`.

### 4.2. Kiểm thử Widget (Widget Test)
* **File:** `test/widget_test.dart`
* **Nội dung:** Kiểm thử Smoke test mặc định của Flutter để đảm bảo cấu trúc widget cơ bản hoạt động (ví dụ: bộ đếm). Cần mở rộng thêm để kiểm thử các màn hình Login hoặc List.

## 5. Hướng dẫn Cài đặt và Chạy ứng dụng

Để giám khảo có thể kiểm tra ứng dụng, vui lòng thực hiện các bước sau:
...
3.  **Cấu hình Firebase:**
    * Đảm bảo máy chạy đã có file cấu hình Firebase (`google-services.json` cho Android hoặc `GoogleService-Info.plist` cho iOS) trong thư mục tương ứng.
...
5.  **Quy trình kiểm tra (Demo):**
    * Đăng ký một tài khoản mới.
    * Đăng nhập vào hệ thống.
    * Thêm một phim mới với đầy đủ thông tin (Ảnh bìa, Số lượng vé, Mô tả...).
    * Bấm vào phim để xem chi tiết.
    * Tại màn hình chi tiết, bấm nút **Edit** để sửa giá vé hoặc tên phim -> Xác nhận thông tin cập nhật ngay lập tức.
    * Quay lại danh sách và xóa phim.
    * Chạy lệnh `flutter test` để xem kết quả kiểm thử tự động.
